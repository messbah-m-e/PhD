...
def detect_and_cordinate(counter):
    ...
    if len(detections) > 0:
        cv2.imwrite(f'final/{counter}_rgb.png', imgs_data[0])
        cv2.imwrite(f'final/{counter}_depth.png', imgs_data[1])
        try:
...
def adjust_altitude():
    while (depth := get_depth()) <= 14:
        print(f"Depth: {depth}\nObject near .... Going Up 2 meters")
        sim.immediate_stop()
    
        time.sleep(1.1)
    
    while (depth := get_depth(all_pixels=True)) >= 22:
        print(f"Depth: {depth}\nObject far .... Going Down 2 meters")
        sim.immediate_stop()
        time.sleep(0.1)
...
drone_ts = sim.get_drone_state()["ts"]
